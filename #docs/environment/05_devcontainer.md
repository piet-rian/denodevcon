...
## コンテナそのものに対して `apt install` およびそれに準ずる行為が必要だと思われる場合

1. devcontainer の [features](https://containers.dev/features/) で代用できないか検討する
2. 代用できない場合は `.devcontainer/postCreateCommand.sh` 内にて相当する行為を行う
3. dockerfile内にて `apt install` は行わないことを基本とする
